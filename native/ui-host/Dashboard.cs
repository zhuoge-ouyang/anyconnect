...
namespace SplitTunnel.UI {
    internal sealed class DashboardForm : Surface {
        ...
        private readonly Label brandFooter;
        ...
        internal DashboardForm(Dictionary<string,object> request):base("AnyConnect 分流管理台",new Size(1008,716),Json.Text(request,"asset_root")) {
            ...
            Picture(sidebar,"app-brand.png",26,82,54,54);Label(sidebar,92,80,160,42,"分流守卫",22,true);
            Label(sidebar,94,121,160,24,"国内直连，按需连接",9);
            ...
            Disconnect=Button(ConnectPage,0,170,228,52,"断开连接",delegate{Send("disconnect");},true);
            Reconnect=Button(ConnectPage,0,170,228,52,"连接 VPN",delegate{Send("reconnect");},true);
            Disconnect.Font=UiFont(14,true);Reconnect.Font=UiFont(14,true);Disconnect.Visible=false;Reconnect.Enabled=false;
            Separator(ConnectPage,246);Label(ConnectPage,0,272,660,26,"连接模式",12,true);
            Domestic=Place(ConnectPage,new RadioButton{Text="国内直连",Font=UiFont(12),ForeColor=Ink,Enabled=false},0,306,140,32);
            VPN=Place(ConnectPage,new RadioButton{Text="VPN 优先",Font=UiFont(12),ForeColor=Ink,Enabled=false},144,306,140,32);
            ModeHint=Label(ConnectPage,0,346,680,30,"默认直连，仅国外白名单中的目标通过 VPN。",10,false,Muted);
            Separator(ConnectPage,386);Label(ConnectPage,0,406,660,30,"需要单独走 VPN？",12,true);
            var inputHost=Place(ConnectPage,new Panel{BackColor=Color.White,BorderStyle=BorderStyle.FixedSingle,Anchor=AnchorStyles.Top|AnchorStyles.Left|AnchorStyles.Right},0,442,548,42);
            ...
            QuickAdd=Button(ConnectPage,564,442,126,42,"添加",SubmitQuick);QuickAdd.Anchor=AnchorStyles.Top|AnchorStyles.Right;QuickAdd.Enabled=false;
            Rules=Button(ConnectPage,0,494,132,34,"查看全部规则",Whitelist);Rules.BackColor=Paper;
            QuickHint=Label(ConnectPage,144,500,536,26,"",9,false,Muted);
            ...
            timer.Tick+=delegate{RefreshState();};Shown+=delegate{RefreshState();timer.Start();Activate();};FormClosed+=delegate{timer.Stop();timer.Dispose();};
        }
        ...
        private void ResizeContent(){if(workspace==null||footer==null)return;float dpi=DeviceScale();int sw=(int)(ClientSize.Width*.24);sidebar.SetBounds(0,0,sw,ClientSize.Height);brandFooter.Top=ClientSize.Height-(int)(88*dpi);workspace.SetBounds(sw+(int)(38*dpi),0,ClientSize.Width-sw-(int)(70*dpi),ClientSize.Height);int content=Math.Max((int)(460*dpi),workspace.Height-(int)(164*dpi));foreach(var p in new[]{ConnectPage,SettingsPage})p.SetBounds(0,(int)(72*dpi),workspace.Width,content);footer.SetBounds(0,workspace.Height-(int)(88*dpi),workspace.Width,(int)(80*dpi));sidebar.Invalidate();}
        private float DeviceScale(){using(var g=CreateGraphics())return g.DpiX/96f;}
        internal void Page(bool settings){SettingsPage.Visible=settings;ConnectPage.Visible=!settings;underline.Left=(int)((settings?207:11)*DeviceScale());}
        ...
        internal void Unavailable(string reason){Latest=null;Status.Text="状态不可用";SiteLabel.Text="当前线路：未确认";Disconnect.Visible=false;Reconnect.Visible=true;foreach(Control c in new Control[]{Reconnect,Smart,Restore,UpdateIPDB,Rules,NavRules,Domestic,VPN,QuickAdd,AutoStart})c.Enabled=false;Hint.Text=reason;Hint.ForeColor=Amber;}
        ...
        internal void Apply(Dictionary<string,object> state){
            ...
            try{
                string status=Json.Text(state,"status_text","状态：未知");bool busy=Regex.IsMatch(status,"正在|初始化|检测中"),connected=Regex.IsMatch(status,"已启用|已连接|已回退静态路由")&&!Regex.IsMatch(status,"未连接|已断开|失败|错误");
                ...
                Disconnect.Enabled=Reconnect.Enabled=!running&&!busy;foreach(Control c in new Control[]{Restore,UpdateIPDB,Rules,NavRules,Domestic,VPN,QuickAdd})c.Enabled=!running;
                ...
                Smart.Enabled=true;AutoStart.Enabled=true;AutoStart.Checked=Json.Flag(state,"auto_start_enabled");bool vpn=Json.Text(state,"split_mode")=="foreign_direct";Domestic.Checked=!vpn;VPN.Checked=vpn;
                ModeHint.Text=vpn?"默认通过 VPN；国内白名单中的目标保持直连。":"默认直连，仅国外白名单中的目标通过 VPN。";
