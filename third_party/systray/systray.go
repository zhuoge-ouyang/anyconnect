...
var (
	...
	iconClickMu   sync.RWMutex
	iconClick     func(left bool)
...
// SetIconClickHandler installs a tray icon click handler.
// The handler receives true for left-click and false for right-click.
// If no handler is installed, the native menu behavior is unchanged.
func SetIconClickHandler(handler func(left bool)) {
	iconClickMu.Lock()
	iconClick = handler
	iconClickMu.Unlock()
}

func dispatchIconClick(left bool) bool {
	iconClickMu.RLock()
	handler := iconClick
	iconClickMu.RUnlock()
	if handler == nil {
